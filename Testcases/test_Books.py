...
def get_csv_data():
    with open('filtered_google_books.csv', mode='r', encoding='utf-8') as f:
        ...
        return [row for row in reader][:5] # Using first 5 books as test samples
...
def test_book_crud_and_rating(client):
    # 1. Setup: Create Book (Public)
    book_resp = client.post("/books/", json={
        "title": "The Great Gatsby", "author": "F. Scott Fitzgerald", "avgRating": 0.0, "genreIds": []
    })
    book_id = book_resp.json()["id"]

    # 2. Setup: Create User & Get Token
    client.post("/users/create", json={"email": "rater@test.com", "password": "password"})
    login_resp = client.post("/users/login", data={"username": "rater@test.com", "password": "password"})
    token = login_resp.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}

    # 3. Rate the Book (Requires Authorization Header)
    rating_data = {"bookId": book_id, "rating": 5.0} # No email/pwd in JSON anymore
...
def test_dynamic_update_and_rating(client):
    ...
    login_resp = client.post("/users/login", data={"username": "critic@test.com", "password": "password"})
    token = login_resp.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
    ...
    # Test Rating (Updated to check message and use headers)
